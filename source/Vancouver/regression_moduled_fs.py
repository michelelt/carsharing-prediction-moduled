...
def run_rfr(best_sol, most_ranked):
	## =============================================================================
	## RFR regression   
	## =============================================================================
    start = time.time()
        ...
        for target in sorted(reg.targets):
            for train_index, valid_index in loo.split(reg.complete_dataset):
                for nof in range(1,84):
    	            ...
    	            print('RFR', target, var, valid_index, nof)
    
    	            reg.set_norm(normed)
    	            reg.split_datasets(target, train_index, valid_index,
                                    features_to_keep=most_ranked.iloc[1, nof])
...
def run_svr(best_sol, most_ranked):
	# =============================================================================
	# SVR regression
	# =============================================================================
    start = time.time()
        ...
        for target in sorted(reg.targets):
            for train_index, valid_index in loo.split(reg.complete_dataset):
                for nof in range(1,84):
    	            ...
    	            print('SVR', target, var, valid_index, nof)
...
if __name__=='__main__':
    ...
    if sys.argv[1].lower() == 'rfr':
        run_rfr(best_sol, most_ranked)
    elif sys.argv[1].lower() == 'svr':
        run_svr(best_sol, most_ranked)
    else:
        print('error')
