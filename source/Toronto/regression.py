...
for c in train.columns:
    ...
    or ('Gi_' in c)\
    or ('t_age') in c: #better drop total age, given that is strongly correlated with m/f age
        row_to_del.append(c)
#
...
s = pd.Series(index=perf_columns)
NOF = 107
for nof in range(1, NOF):
...
#            print()
            ...
            s['y_pred'] = pd.Series(index=index2FID_test.values, data=y_pred).to_csv()
            s['y_test'] = pd.Series(index=index2FID_test.values, 
                                     data=y_test.values.tolist()).to_csv()
...
perf_df.to_csv('../../data/Vancouver/Regression/Vancouver_perf.csv')
